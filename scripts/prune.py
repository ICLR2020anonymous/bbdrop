...
from utils.misc import accuracy, freeze_batch_norm

parser = argparse.ArgumentParser()
parser.add_argument('--config', default='configs/lenet_mlp_mnist_vib.py', type=str)
...
def test(load=True, logger=None, epoch=None):
    ...
    logger.info('reg {:.4f}'.format(net.get_reg().item()))
    logger.info('pruned size {}'.format(str(net.get_pruned_size())))
    logger.info('speedup in flops {:.4f}'.format(net.get_speedup()))
    logger.info('memory saving {:.4f}\n'.format(net.get_memory_saving()))
